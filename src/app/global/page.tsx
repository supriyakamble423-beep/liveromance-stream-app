...
import { MessageCircle, Zap, ShieldCheck, Lock, TrendingUp, RefreshCw, Sparkles, X, Heart } from "lucide-react";
      ...
      <main className="px-4 pt-6 space-y-6">
        {showAIBot && (
          <div className="relative animate-in slide-in-from-right-10 duration-500">
            <div className="romantic-gradient p-5 rounded-[2rem] text-white shadow-2xl relative overflow-hidden group romantic-glow">
        ...
        <section className="bg-primary/5 border border-primary/10 rounded-[2rem] p-5 flex items-center justify-between romantic-card-glow">
          <div className="space-y-1">
            <p className="text-[10px] font-black uppercase tracking-widest text-primary">Global Love Nodes</p>
            <h2 className="text-xl font-black font-headline tracking-tighter uppercase italic">Market Activity</h2>
          </div>
          <div className="flex gap-4 text-right">
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase">Online</p>
              <p className="text-lg font-black text-primary">{hosts?.length || 0}</p>
            </div>
            <div>
              <p className="text-xs font-bold text-slate-400 uppercase">Hearts</p>
        ...
        <section className="flex items-center justify-between">
          <div className="flex flex-col">
            ...
            <p className="text-[10px] text-muted-foreground uppercase tracking-widest font-bold flex items-center gap-1">
              <Heart className="size-3 text-secondary fill-current" /> Real-time Global Traffic
            </p>
          </div>
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-20 space-y-4 opacity-50">
            ...
            <p className="text-[10px] font-black uppercase tracking-widest">Syncing Love Nodes...</p>
          </div>
        ) : (
              ...
              <div key={host.id} className="flex flex-col bg-card rounded-[2.5rem] overflow-hidden border border-border group transition-all hover:shadow-2xl relative romantic-card-glow">
                <div className="relative aspect-[3/4] overflow-hidden bg-muted">
                  ...
                  <div className="absolute top-4 left-4 flex flex-col gap-2">
                    <Badge className="bg-primary border-none text-[9px] font-black uppercase tracking-widest px-3">Live</Badge>
                  </div>
                  {host.streamType === 'private' && (
                    <div className="absolute inset-0 bg-black/60 backdrop-blur-[4px] flex flex-col items-center justify-center gap-2">
                      <Lock className="size-8 text-secondary/60" />
                      <p className="text-[10px] font-black text-white/60 uppercase tracking-widest">Private Room</p>
                    </div>
                  )}
                </div>
                <div className="p-4 space-y-4">
                  ...
                  <div className="flex gap-2">
                    <Button 
                      ...
                      className="flex-1 romantic-gradient rounded-2xl h-10 text-[10px] font-black gap-1 uppercase tracking-widest text-white hover:scale-105 transition-transform romantic-glow"
                    >
